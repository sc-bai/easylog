...
class LevelHelper : base::StaticClass {
 public:
  ...
  static const base::type::EnumType kMaxValid = static_cast<base::type::EnumType>(Level::UDP);
...
inline void FUNCTION_NAME(const T&);
  ...
  LOGGER_LEVEL_WRITERS_SIGNATURES(udp)
...
LOGGER_LEVEL_WRITERS(udp, Level::UDP)
...
#define TIMED_BLOCK(obj, blockName) for (TIME_BLOCK_PERFORMANCE_TRACKER obj = { 0, \
    el::base::PerformanceTrackerPtr(new el::base::PerformanceTracker(blockName, ELPP_MIN_UNIT)) }; obj.i < 1; ++obj.i)
     */
...
#define CUDP(writer, dispatchAction, ...) ELPP_WRITE_LOG(writer, el::Level::UDP, dispatchAction, __VA_ARGS__)
#if ELPP_INFO_LOG
